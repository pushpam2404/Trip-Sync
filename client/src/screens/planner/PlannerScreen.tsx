...
export const PlannerScreen = () => {
    const {
        savedRoutes, saveRoute, removeRoute, setActiveTab,
    } = useAppContext();
        ...
        const fetchStays = async () => {
            if (step === 2 && !hasStayPlanned && destination && destination !== 'Current Location') {
                ...
                if (results && results.length > 0) {
                    ...
                    setStaysError('Could not find any stays nearby. Try another destination.');
                }
    ...
    const handleDestinationNext = async () => {
        ...
        if (destination === 'Current Location') {
            ...
            try {
                ...
                if (error?.code === 1) errorMessage = "Permission denied. Enable location access.";
    ...
    const handleVoiceSearch = async (field: 'destination' | 'stay') => {
        ...
        if (!SpeechRecognition) {
            setMicPermissionError("Speech recognition is not supported in this browser.");
            return;
        }
        ...
        recognition.onstart = () => setIsListeningFor(field);
        recognition.onend = () => {
            setIsListeningFor(null);
            recognitionRef.current = null;
        };
        recognition.onerror = () => setIsListeningFor(null);
        ...
        <div className="flex items-center justify-between max-w-[240px] mx-auto mb-6 select-none">
            {[1, 2, 3].map(num => (
                <div key={num} className="flex items-center">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs border transition-all duration-200 ${step === num ? 'bg-cyan-500 border-cyan-500 text-slate-950 shadow-sm font-extrabold' : step > num ? 'bg-slate-900 border-cyan-500 text-cyan-400 font-bold' : 'bg-slate-900 border-slate-800 text-slate-550'}`}>
                        {num}
                    </div>
                    {num < 3 && (
                        <div className={`h-px w-10 mx-2 transition-all duration-200 ${step > num ? 'bg-cyan-500' : 'bg-slate-800'}`}></div>
                    )}
                </div>
            ))}
        </div>
    ...
    if (step === 3) {
        return (
            <>
                ...
                <div className="p-6 text-slate-100 max-w-sm w-full mx-auto space-y-5 animate-fade-in">
                    <StepIndicator />
                    
                    <div className="mb-2 text-center">
                        <h1 className="text-lg font-bold text-white tracking-tight">Suggestions for {destination}</h1>
                        <p className="text-xs text-slate-400 mt-1 leading-snug">Select popular attractions near your stay ({stayName}) to save to your routes list.</p>
                    </div>

                    {isLoadingAttractions && (
                        <div className="flex flex-col items-center justify-center py-10 space-y-3">
                            <div className="w-8 h-8 rounded-full border-2 border-cyan-500 border-t-transparent animate-spin"></div>
                            <p className="text-slate-405 text-xs">Finding attractions near stay...</p>
                        </div>
                    )}
                    ...
                    {!isLoadingAttractions && !attractionsError && nearbyAttractions.length === 0 && (
                        <p className="text-center text-slate-500 text-xs py-10 italic">No popular attractions found in this area.</p>
                    )}

                    <div className="space-y-3 overflow-y-auto max-h-[42vh] pr-1.5 scrollbar-thin">
                        {nearbyAttractions.map(dest => {
                            const isAdded = savedRoutes.some(r => r.origin === stayName && r.destination === dest.name);
                            return (
                                <div key={dest.id} className="card bg-slate-900 border border-slate-850 p-3 flex justify-between items-center hover:border-slate-800 transition-colors">
                                    <div className="flex items-center space-x-3 overflow-hidden">
                                        <img src={dest.image} alt={dest.name} className="w-12 h-12 rounded-lg object-cover flex-shrink-0 bg-slate-800 border border-slate-700/50" />
                                        <div className="overflow-hidden space-y-0.5">
                                            <h3 className="font-bold text-white text-xs truncate" title={dest.name}>{dest.name}</h3>
                                            <div className="flex items-center space-x-1.5">
                                                {dest.rating && dest.rating > 0 ? (
                                                    <span className="text-[10px] font-bold text-amber-400">{dest.rating.toFixed(1)} ★</span>
                                                ) : null}
                                                <span className="text-[9px] font-bold text-cyan-400 uppercase tracking-wide">Attraction</span>
                                            </div>
                                            <p className="text-[9px] text-slate-500 truncate" title={dest.distance}>{dest.distance}</p>
                                        </div>
                                    </div>
                                    <button
                                    ...
                                    >
                                        {isAdded ? <CheckIcon className="h-4 w-4 text-white" /> : <PlusIcon className="h-4 w-4" />}
                                    </button>
                                </div>
                            );
                        })}
                    ...
                    <div className="space-y-3 pt-3">
                        <button onClick={completeAndGoHome} className="btn btn-primary btn-full py-3">Complete Planning</button>
                        <button onClick={() => setActiveTab('destinations')} className="btn btn-secondary btn-full py-3">View Saved Destinations</button>
                    </div>
                </div>
            </>
        );
    ...
    if (step === 2) {
        return (
            <>
                <TabScreenHeader title="Trip Planner" />
                <div className="p-6 text-slate-100 max-w-sm w-full mx-auto space-y-6 animate-fade-in">
                    <StepIndicator />

                    <div className="card bg-slate-900 border border-slate-800 p-4 flex justify-between items-center shadow-sm">
                        <label htmlFor="stay-toggle" className="text-xs font-semibold text-slate-300">Have you booked a stay?</label>
                        <button
                        ...
                        >
                            <span className={`w-4 h-4 bg-slate-950 rounded-full shadow transform transition-transform absolute ${hasStayPlanned ? 'translate-x-5' : 'translate-x-1'}`}></span>
                        </button>
                    </div>

                    {hasStayPlanned ? (
                        <div className="space-y-2">
                            <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Stay Location Name</label>
                            <div className="relative">
                                <input
                                    ...
                                    onBlur={() => setTimeout(() => setActivePlannerInput(null), 250)}
                                ...
                                <button
                                    ...
                                    className={`absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-md transition-all cursor-pointer ${isListeningFor === 'stay' ? 'bg-rose-500/10 text-rose-500 animate-pulse' : 'text-slate-500 hover:text-slate-300'}`}
                        ...
                        <div className="space-y-4">
                            <div className="space-y-1 text-center">
                                <h3 className="text-sm font-bold text-white">Select a stay in {destination}</h3>
                                <p className="text-xs text-slate-400">Showing top rated lodgings & accommodations nearby</p>
                            </div>
                            
                            {isLoadingStays && (
                                <div className="flex flex-col items-center justify-center py-10 space-y-3">
                                    <div className="w-8 h-8 rounded-full border-2 border-cyan-500 border-t-transparent animate-spin"></div>
                                    <p className="text-slate-405 text-xs">Searching accommodations...</p>
                                </div>
                            )}
                            ...
                            <div className="space-y-3 overflow-y-auto max-h-[34vh] pr-1.5 scrollbar-thin">
                                {stays.map(stay => (
                                    <div key={stay.id} className="card bg-slate-900 border border-slate-850 p-3 flex justify-between items-center hover:border-slate-800 transition-colors">
                                        <div className="flex items-center space-x-3 overflow-hidden">
                                            <img src={stay.image} alt={stay.name} className="w-12 h-12 rounded-lg object-cover flex-shrink-0 bg-slate-800 border border-slate-700/50" />
                                            <div className="overflow-hidden space-y-0.5">
                                                <h3 className="font-bold text-white text-xs truncate">{stay.name}</h3>
                                                <div className="flex items-center space-x-1.5">
                                                    <span className="text-[10px] font-bold text-amber-400">{stay.rating} ★</span>
                                                    <span className="text-[9px] font-bold text-emerald-400 uppercase tracking-wide">Hotel</span>
                                                </div>
                                                <p className="text-[9px] text-slate-500 truncate" title={stay.distance}>{stay.distance}</p>
                                            </div>
                                        </div>
                                        <button
                    ...
                    <button 
                        ...
                        disabled={(hasStayPlanned && !stayLocation) || (!hasStayPlanned && !selectedStayId)} 
    ...
    return (
        <>
            ...
            <div className="p-6 text-slate-100 max-w-sm w-full mx-auto space-y-6 animate-fade-in">
                ...
                <div className="space-y-4">
                    <div className="space-y-1 text-center">
                        <label className="block text-sm font-bold text-white tracking-tight">Destination City</label>
                        <p className="text-xs text-slate-400">Tell Sakha where we are going to explore stays and landmarks</p>
                    </div>
                    ...
                    <div className="relative">
                        ...
                        <button
                            ...
                            className={`absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-md transition-all cursor-pointer ${isListeningFor === 'destination' ? 'bg-rose-500/10 text-rose-500 animate-pulse' : 'text-slate-500 hover:text-slate-300'}`}
