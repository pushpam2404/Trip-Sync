...
import { MicrophoneIcon, ReverseIcon, PaperPlaneIcon, ArrowRightIcon } from '../constants';
...
export const SakhaScreen = () => {
    ...
    const [activeMode, setActiveMode] = useState<'quick' | 'assistant'>('quick');
    ...
    const handleVoiceSearch = async (field: 'origin' | 'destination') => {
        ...
        try {
            ...
            setMicPermissionError("Microphone access denied. Please check permissions.");
            return;
        }
        ...
        recognition.onresult = (event: any) => {
            ...
            if (field === 'origin') setNavigationOrigin(transcript);
            else setNavigationDestination(transcript);
        };
        recognition.start();
    };
    ...
    const handleStartTrip = async () => {
        ...
        const resolveCurrentLocation = async (field: 'origin' | 'destination') => {
            ...
            try {
                ...
                setPermissionError("Could not resolve location. Please configure settings or search manually.");
    ...
    return (
        <>
            ...
            <div className="p-6 text-slate-100 flex flex-col h-[calc(100vh-140px)] max-w-sm w-full mx-auto space-y-5 animate-fade-in">
                    ...
                    <div className="flex-grow flex flex-col justify-between space-y-4 overflow-y-auto no-scrollbar">
                        <div className="card bg-slate-900 border border-slate-800 p-5 space-y-5 relative shadow-sm">
                            <div className="relative">
                                <label htmlFor="origin-input" className="block text-[11px] font-bold text-slate-450 uppercase tracking-wider mb-2">Starting Point</label>
                                <div className="relative">
                                    <input
                                        id="origin-input"
                                        type="text"
                                        ...
                                        onFocus={() => {
                                            if (navigationOrigin === 'Current Location') setNavigationOrigin('');
                                    ...
                                    <button
                                        ...
                                        className={`absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-md transition-all cursor-pointer ${isListeningFor === 'origin' ? 'bg-rose-500/10 text-rose-500 animate-pulse' : 'text-slate-500 hover:text-slate-300'}`}
                            ...
                            <div>
                                <label htmlFor="destination-input" className="block text-[11px] font-bold text-slate-450 uppercase tracking-wider mb-2">Final Destination</label>
                                <div className="relative">
                                    <input
                        ...
                        <div className="flex-grow overflow-y-auto space-y-4 pr-1 pb-4 scrollbar-thin">
                            {chatMessages.map(msg => (
                                <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} space-y-1.5`}>
                                    <div className={`max-w-[85%] px-4 py-3 rounded-lg text-xs leading-relaxed ${msg.role === 'user' ? 'bg-slate-800 border border-slate-700 text-white rounded-tr-none' : 'bg-slate-900 border border-slate-800 text-slate-200 rounded-tl-none'}`}>
                                        {msg.text.split('\n').map((line, i) => (
                                    ...
                                    {msg.navigationIntent && (
                                        <div className="card bg-slate-900 border border-slate-800 p-4 flex flex-col gap-3.5 w-full max-w-[85%] mt-1">
                                            <p className="text-[10px] font-bold text-cyan-400 uppercase tracking-wider">Plan Detected</p>
                                            <p className="text-xs font-medium text-slate-200 leading-snug">Start road trip from {msg.navigationIntent.origin} to {msg.navigationIntent.destination}?</p>
                                            <button
                                                onClick={() => startNavigationFrom(msg.navigationIntent!.origin, msg.navigationIntent!.destination)}
                                                className="btn btn-primary btn-sm flex items-center justify-center gap-1.5 cursor-pointer text-xs py-2 px-3"
                                            >
                                                Start Navigation <ArrowRightIcon className="w-3.5 h-3.5" />
                                            </button>
                                        </div>
                                    )}
                                </div>
